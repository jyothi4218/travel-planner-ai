...
const Timeline = ({ itinerary, planId, allowEdit, fromDate }: TimelineProps) => {
  const [activeBooking, setActiveBooking] = useState<{ name: string; date: string } | null>(null);
  ...
  return (
    <>
      {activeBooking && (
        <HotelBookingModal
          hotelName={activeBooking.name}
          destination={filteredItinerary?.[0]?.title ?? ""}
          checkIn={activeBooking.date}
          checkOut={activeBooking.date}
      ...
      <div className="flex flex-col gap-4 mt-4">
        {filteredItinerary?.map((day, index) => {
          const dayDate = fromDate
            ...
            : null;
          const dayDateISO = fromDate
            ? new Date(fromDate + index * 24 * 60 * 60 * 1000).toISOString().split("T")[0]
            : "";
              ...
              <div className="flex items-center gap-4 p-4 border-b border-border">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    ...
                    {(day as any).daytheme && (
                      <span className="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 rounded-full font-medium">
                        {(day as any).daytheme}
                      </span>
                    )}
                    {(day as any).estimateddailycost && (
                        ...
                        if (booking.type === "hotel") {
                          return (
                            <button
                              key={i}
                              type="button"
                              onClick={() => setActiveBooking({ name: booking.name, date: dayDateISO })}
                              className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border transition-all hover:opacity-80 bg-green-50 text-green-700 border-green-200 dark:bg-green-950/30 dark:text-green-300"
                            >
                              <Hotel className="w-3 h-3" />
                              {booking.name}
                              <span className="ml-1 font-semibold">Book Now</span>
                            </button>
                          );
                        }
                        return (
                          <a
                            ...
                            className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border transition-all hover:opacity-80 bg-pink-50 text-pink-700 border-pink-200 dark:bg-pink-950/30 dark:text-pink-300"
                          >
